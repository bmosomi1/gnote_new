...
def water_revenues(request):
    #customer = Customer.objects.filter(user_ptr_id=request.user.id).first()
    customer = WaterClientAll.objects.get(id=1)
    if customer is None:
        customer = WaterClientAll.objects.get(id=1)
        weeks = get_last_n_weeks(12)
        months = get_last_n_months(12)
...
def meter_readings_report(request):
    ...
    if request.method == 'POST':
        ...
        if report_value=='1':
            ...
            cell.value = 'GREEEN NOTE WATER CLIENTS' 
            ...
            summary_sheet.append(('GREEN NOTE WATER CLIENTS','NAMES'))
            big_red_text = Font(color="00FF0000", size=20)
            sheet["B1"].font = big_red_text
            sheet["B2"].font = big_red_text
            summary_sheet.append(('A/C','NAMES', 'PHONE NUMBER', 'ID NUMBER', 'COURT','REG DATE', 'READINGS','AMOUNT DUE'))
            ...
            for cust in customer:
                #summary_sheet.append((cust.id, cust.names, cust.last_meter_reading))
                summary_sheet.append((cust.id, cust.names, cust.msisdn,cust.id_num,cust.court,cust.created_at ,cust.last_meter_reading,cust.amount_due))
                number += 1
        ...
        if report_value=='4':
            ...
            #cell.alignment = Alignment(horizontal='center', vertical='center')
            #summary_sheet.merge_cells('A1:B1')
            summary_sheet.append(('PAYMENT DATE','A/C NUMBER', 'ACCOUNT NAME', 'AMOUNT', 'PAID BY','CONFIRMATION CODE', 'OUTSTANDING BALANCE'))

            number = 1
            for cust in customer:
                #summary_sheet.append((cust.id, cust.names, cust.last_meter_reading))
                summary_sheet.append((cust.pay_date, cust.account_number, cust.account_name,cust.amount,cust.received_from,cust.confirmation_code ,cust.balance_carried_forward))
                number += 1
        ...
        if report_value=='5':
            ...
            summary_sheet.append(('A/C','NAMES', 'PHONE NUMBER' ,'CURRENT ARREARS', '1 MONTH OLD', '2 MONTHS OLD','3 MONTHS', 'OVER 3 MONTHS','TOTAL ARREARS'))
            ...
            for cust in customer:
                ...
                summary_sheet.append((cust.id, cust.names, cust.msisdn,cust.amount_due,m1_arrears,m2_arrears,m3_arrears,m4_arrears ,cust.amount_due))
                number += 1
...
def main_meter_replacement(request):
    ...
    if request.method == 'POST':
        ...
        client_id = request.POST['meter']
        meter_number = request.POST['meter_number']
        customer = MainMeter.objects.filter(id=1).first()
        #customer = WaterClientAll.objects.filter(id=client_id).first()
        ...
        MainMeterReplacement.objects.create(
            client=customer,
            last_units=last_readings,
            comments=comments,
        ...
        context = {
            'replacements': MainMeterReplacement.objects.filter().order_by('-id'),
...
def water_payments_allocations2(request):
